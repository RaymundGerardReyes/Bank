...
const PROJECT_NAME = process.argv[2] || 'banking';
const REGISTRY_FILE = path.join(__dirname, '..', 'PORT_REGISTRY.md');
const COMPOSE_DIR = path.join(__dirname, '..', 'infra', 'docker');
...
function parseDockerCompose() {
    ...
    const cmd = `npx dotenvx run --env-file="${DOTENV_PATH}" -q -- docker compose -f compose.yaml -f compose.dev.yaml config --format json`;
