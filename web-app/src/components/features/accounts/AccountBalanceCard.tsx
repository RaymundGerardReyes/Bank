...
export const AccountBalanceCard: React.FC<AccountBalanceCardProps> = ({ account }) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="bg-dominant border border-secondary/30 rounded-xl p-5 shadow-sm mt-1">
        ...
        <div className="grid grid-cols-3 gap-2 py-3 border-t border-secondary/20 mb-2">
          <div className="flex flex-col">
            <span className="text-[9px] text-accent/50 uppercase font-bold tracking-wider mb-0.5">Hierarchy</span>
            <span className="text-[11px] font-bold text-accent truncate">
              {account.parentAccountId ? `Sub-Ledger` : `Master Ledger`}
            </span>
          </div>
          <div className="flex flex-col border-l border-secondary/20 pl-3">
            <span className="text-[9px] text-accent/50 uppercase font-bold tracking-wider mb-0.5">Account Type</span>
            <span className="text-[11px] font-bold text-accent truncate">{account.accountType || "MAIN"}</span>
          </div>
          <div className="flex flex-col border-l border-secondary/20 pl-3">
            <span className="text-[9px] text-accent/50 uppercase font-bold tracking-wider mb-0.5">Isolation</span>
            <span className="text-[11px] font-bold text-emerald-600 truncate">{account.parentAccountId ? "Isolated" : "Root"}</span>
          </div>
        </div>

        {/* Actions */}
        <div className="flex items-center gap-2 pt-3 border-t border-secondary/20 mt-1">
          <button
            onClick={() => router.push("/transactions/history")}
            className="flex-1 text-xs font-bold text-accent bg-surface hover:bg-secondary/20 py-2.5 rounded-lg transition-colors border border-secondary/30"
          >
            History
          </button>
          <button
            ...
            className="flex-1 text-xs font-bold text-dominant bg-accent hover:bg-accent/90 disabled:bg-secondary disabled:cursor-not-allowed py-2.5 rounded-lg transition-colors shadow-md"
          >
            Transfer
          </button>
        </div>
