...
export default function OpsLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-surface text-accent overflow-hidden">
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex flex-col w-64 bg-dominant/90 backdrop-blur-md border-r border-secondary/20 shadow-md z-20">
        <div className="p-6 border-b border-secondary/20 bg-accent text-dominant">
          <Link href="/ops-dashboard" className="text-xl font-extrabold tracking-tight flex items-center gap-2 group">
            <div className="w-8 h-8 bg-dominant rounded-lg text-accent flex items-center justify-center text-sm font-black shadow-sm transition-transform duration-200 group-hover:scale-105">
              N
            </div>
            NovaBank
          </Link>
              ...
              <div className="flex flex-col gap-1">
                {group.items.map((item) => {
                  ...
                  const isActuallyActive =
                    pathname === actualPath || (actualPath !== "/" && pathname.startsWith(actualPath));

                  return (
                    <Link
                      key={item.href}
                      href={actualPath}
                      className={`relative px-3.5 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 flex items-center justify-between group ${
                        isActuallyActive
                          ? "bg-secondary/15 text-accent font-extrabold shadow-sm"
                          : "text-accent/60 hover:bg-secondary/10 hover:text-accent"
                      }`}
                    >
        ...
        <div className="p-4 border-t border-secondary/20 bg-secondary/5">
          <div className="flex items-center gap-3 mb-4 px-2">
            <div className="w-8 h-8 rounded-full bg-accent text-dominant flex items-center justify-center font-bold shadow-sm">
          ...
          <button
            ...
            className="w-full py-2.5 px-4 text-xs font-bold text-rose-600 border border-rose-200 rounded-xl hover:bg-rose-50 active:scale-95 transition-all shadow-sm"
          >
            Sign Out
          </button>
        </div>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="md:hidden flex items-center justify-between p-4 bg-accent text-dominant z-20 shadow-md">
        ...
        <main className="flex-1 overflow-y-auto bg-surface relative">
          <PageTransition className="max-w-7xl mx-auto p-6 md:p-10 min-h-full">
            {children}
          </PageTransition>
        </main>
      </div>
