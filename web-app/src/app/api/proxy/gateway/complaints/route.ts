...
// GET  /api/proxy/gateway/complaints                 → list all
// POST /api/proxy/gateway/complaints/[ref]/resolve   → resolve a complaint

export async function GET(request: Request) {
  const { token, requestId } = extractToken(request);

  try {
    const res = await fetch(`${env.backendApiBaseUrl}/api/v1/gateway/complaints`, {
      headers: {
