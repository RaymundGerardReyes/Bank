...
// GET  /api/proxy/governance/requirements              → list all requirements
// POST /api/proxy/governance/requirements/[id]/evidence → generate evidence
...
export async function GET(request: Request) {
  ...
  try {
    const res = await fetch(
      `${env.backendApiBaseUrl}/api/v1/governance/regulatory-requirements`,
      {
        headers: {
