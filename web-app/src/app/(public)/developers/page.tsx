...
const API_MODULES = [
  ...
  },
  {
    ...
    endpoints: [
      { method: "POST" as HttpMethod, path: "/api/v1/payments", scope: "payments:write", payload: { amount: 1000.00, currency: "USD", description: "Order #12345" } },
      { method: "GET" as HttpMethod, path: "/api/v1/payments/{id}", scope: "payments:read" },
      { method: "POST" as HttpMethod, path: "/api/v1/payments/{id}/authorize", scope: "payments:write", payload: { paymentMethod: "tokenized_card_123" } },
      { method: "POST" as HttpMethod, path: "/api/v1/payments/{id}/capture", scope: "payments:write" },
      { method: "POST" as HttpMethod, path: "/api/v1/payments/{id}/cancel", scope: "payments:write" },
      { method: "POST" as HttpMethod, path: "/api/v1/payments/{id}/expire", scope: "payments:write" },
      { method: "POST" as HttpMethod, path: "/api/v1/payments/{id}/refund", scope: "payments:write", payload: { amount: 1000.00, reason: "customer_requested" } },
      { method: "GET" as HttpMethod, path: "/api/v1/payments/{id}/transactions", scope: "payments:read" },
      { method: "GET" as HttpMethod, path: "/api/v1/payments/{id}/events", scope: "payments:read" }
    ]
  },
  {
    ...
    endpoints: [
      { method: "GET" as HttpMethod, path: "/api/v1/settlements/{id}", scope: "settlements:read" },
      { method: "GET" as HttpMethod, path: "/api/v1/settlements/{id}/instructions", scope: "settlements:read" },
      { method: "POST" as HttpMethod, path: "/api/v1/settlements/{id}/reconcile", scope: "settlements:write", payload: { externalReportId: "rep_9999" } },
      { method: "POST" as HttpMethod, path: "/api/v1/settlements/{id}/execute", scope: "settlements:write" }
    ]
  },
  {
...
export default function DevelopersPage() {
  ...
  const generateCodeSnippet = (method: HttpMethod, path: string, payload?: object) => {
    ...
    if (path.includes("{id}")) {
        const idPrefix = path.includes("settlements") ? "stl_" : "pay_";
