...
# automatic backup branch, Git index self-healing, and full run log.
#
# Usage:
...
if [ ! -f "$CONFIG_FILE" ]; then
  ...
  echo "  scope|tag_prefix|commit_type|commit_description|path1,path2|optional_body"
  exit 1
fi
...
process_module() {
  ...
  for p in "${paths[@]}"; do
    if [ -n "$(git status --porcelain -- "$p" 2>/dev/null)" ] || \
       [ -n "$(git ls-files -m -o -d --exclude-standard "$p" 2>/dev/null)" ]; then
      changes="yes"
      break
    fi
  done
  ...
  if [ "$DRY_RUN" = true ]; then
    log "[DRY-RUN] Would run: git add -A -- ${paths[*]}"
    if [ -n "$commit_body" ]; then
      log "[DRY-RUN] Would run: git commit -m \"${full_commit_msg}\" -m \"${commit_body}\""
    else
  ...
  if [ "$DRY_RUN" = false ]; then
    git add -A -- "${paths[@]}"
    if [ -n "$commit_body" ]; then
      git commit -m "$full_commit_msg" -m "$commit_body" | tee -a "$LOG_FILE"
    else
  ...
  if [[ "$commit_type" != "chore" && "$commit_type" != "docs" && "$commit_type" != "test" ]]; then
    ...
    if [ "$DRY_RUN" = true ]; then
      ...
      if confirm "Create tag $next_tag for this commit?"; then
        local tag_msg="${tag_prefix} Release ${next_version}
...
while IFS= read -u 3 -r line || [ -n "$line" ]; do
  ...
  IFS='|' read -r scope tag_prefix commit_type commit_desc paths_csv commit_body <<< "$line"
  process_module "$scope" "$tag_prefix" "$commit_type" "$commit_desc" "$paths_csv" "$commit_body"
done 3< "$CONFIG_FILE"
